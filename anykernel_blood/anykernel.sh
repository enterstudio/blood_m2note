...
dump_boot() {
  ...
  if [ $? != 0 ]; then
    ui_print " "; ui_print "Dumping/splitting image failed. Aborting..."; echo 1 > /tmp/anykernel/exitcode; exit;
  fi;
  ...
  if [ $? != 0 -o -z "$(ls $ramdisk)" ]; then
    ui_print " "; ui_print "Unpacking ramdisk failed. Aborting..."; echo 1 > /tmp/anykernel/exitcode; exit;
  fi;
...
write_boot() {
  ...
  if [ $? != 0 ]; then
    ui_print " "; ui_print "Repacking ramdisk failed. Aborting..."; echo 1 > /tmp/anykernel/exitcode; exit;
  fi;
  ...
  if [ $? != 0 ]; then
    ...
    ui_print " "; ui_print "New image larger than boot partition. Aborting..."; echo 1 > /tmp/anykernel/exitcode; exit;
  fi;
  if [ -f "/data/custom_boot_image_patch.sh" ]; then
    ash /data/custom_boot_image_patch.sh /tmp/anykernel/boot-new.img;
    if [ $? != 0 ]; then
      ui_print " "; ui_print "User script execution failed. Aborting..."; echo 1 > /tmp/anykernel/exitcode; exit;
    fi;
  fi;
